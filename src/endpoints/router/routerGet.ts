...
export class RouterGet extends OpenAPIRoute {
	schema = {
		...
		responses: {
			"200": {
				...
				...contentJson(z.object({ qr_string: z.string() })),
			},
	...
	async handle(c: AppContext) {
		...
		if (qr && qr.expiration_date > now) {
			return c.json({ qr_string: qr.qr_string });
		}

		// Fallback
		const fallback = await c.env.DB.prepare(
			"SELECT qr_string FROM qrs WHERE user_id = ? AND is_fallback = 1 AND expiration_date > ?",
		)
			.bind(user.id, now)
		...
		return c.json({ qr_string: fallback.qr_string });
	}
}
